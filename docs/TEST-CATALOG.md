...
| 23b | `should_match_visual_baseline_for_follow_us_icons_row`        | Footer Follow Us social-icons row pixel-matches baseline         | `toHaveScreenshot` scoped to Facebook link's parent                     | Stable region (years between icon updates) — catches CSS rebrand regressions | [visual.spec.ts](../tests/visual.spec.ts)           | P3  |
| 39  | `should_position_logo_left_search_center_cart_right`          | Logo left-half, cart right-half, search between them in header   | `boundingBox()` math on three elements                                  | Catches layout reorder/break without pixel-fragile snapshots                 | [visual.spec.ts](../tests/visual.spec.ts)           | P1  |
...
| 8b  | `should_expose_at_least_2_interactive_items_in_opened_*_panel` (×5)             | Each opened mega-menu panel exposes ≥ 2 interactive affordances (link or button) | Hover + count `getByRole('link')` and `getByRole('button')` scoped via `aria-controls`               | Catches "marketing pushed empty data" regressions across panel types                  | [mega-menu.spec.ts](../tests/mega-menu.spec.ts)                 | P1  |
| 8c  | `should_have_no_dead_hrefs_inside_any_opened_mega_menu`             | Every link inside every opened mega-menu has a non-placeholder href              | Hover each trigger; DOM-scan the panel for `#`/`javascript:`/empty hrefs                             | Catches CMS placeholder hrefs that only render after hover                            | [mega-menu.spec.ts](../tests/mega-menu.spec.ts)                 | P1  |
| 8d  | `should_expose_start_designing_and_explore_templates_in_design_lab` | Design Lab panel exposes "Start Designing" + "Explore Templates" CTAs            | Role-based locators within the opened panel                                                          | High-conversion CTAs whose absence is direct revenue regression                       | [mega-menu.spec.ts](../tests/mega-menu.spec.ts)                 | P2  |
...
| 38  | `should_open_avatar_dropdown_with_signin_and_create_account_when_signin_hovered` | Hover on Sign-In opens panel exposing both Sign-In and Create-An-Account actions | `signInLink.hover()` + role-based locators in panel | Loss of "Create An Account" cuts off the acquisition funnel from header | [user-state.spec.ts](../tests/user-state.spec.ts) | P1  |
...
| 37a | `should_serve_robots_txt_with_user_agent_rules`                         | `robots.txt` is reachable and declares User-agent rules               | `request.get('/robots.txt')` + body regex                    | Catches robots.txt 404 / HTML misroute (silent SEO regression)                  | [seo-discovery.spec.ts](../tests/seo-discovery.spec.ts) | P2  |
| 37b | `should_either_declare_sitemap_or_intentionally_block_crawling`         | `robots.txt` declares Sitemap OR blocks all crawling                  | regex on body                                                | Production must publish Sitemap; staging must block all — third state is broken | [seo-discovery.spec.ts](../tests/seo-discovery.spec.ts) | P2  |
| 37c | `should_serve_a_reachable_sitemap_at_a_conventional_path`               | One of `/sitemap.xml`, `/sitemap_index.xml`, `/sitemap` returns < 400 | Iterate candidate paths                                      | Catches broken sitemap that hides organic-traffic loss for weeks                | [seo-discovery.spec.ts](../tests/seo-discovery.spec.ts) | P2  |
