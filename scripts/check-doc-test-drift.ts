...
/** Sections that are intentionally not test-mapped (identity / reference snapshots). */
...
/** Captures the first-line title of a `test.describe(...)` call. */
const DESCRIBE_RE = /test\.describe\(\s*([`'"])((?:\\.|(?!\1).)*?)\1/;
...
function readDocSections(repoRoot: string): DocSection[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match && match[1]) {
      out.push({ id: match[1], heading: lines[i] ?? "", line: i + 1 });
    }
  }
  return out;
}
...
function main(): void {
  ...
  const untested = docSections.filter(
    (s) => !testRefIds.has(s.id) && !UNTESTED_ALLOWLIST.has(s.id),
  );
